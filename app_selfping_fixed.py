...
# ================= SELF-PING WATCHDOG =================
# Keeps an HTTP health endpoint reachable. This is service monitoring only;
# it does not refresh or bypass any third-party account/session authentication.
...
def self_ping_worker():
    ...
    print(f"[SELF-PING] Enabled: every {SELF_PING_INTERVAL}s -> {health_url}", flush=True)
    ...
    while True:
        try:
            response = requests.get(
                health_url,
                timeout=15,
                headers={"User-Agent": "ScarDec-HealthMonitor/1.0"}
            )
            print(
                f"[SELF-PING] HTTP {response.status_code} -> {health_url}",
                flush=True
            )
...
# Start self-ping after all Flask routes (including /health) are registered.
threading.Thread(target=self_ping_worker, name="self-ping", daemon=True).start()
